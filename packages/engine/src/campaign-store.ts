...
import { AURENDOR_ORGANIZATION_ID, type DatabaseClient } from "@aurendor/db/runtime";
...
async function audit(db: DatabaseClient, id: string, actor: string, before: unknown, after: unknown): Promise<void> {
  ...
  [randomUUID(), AURENDOR_ORGANIZATION_ID, actor, id, JSON.stringify(before), JSON.stringify(after), randomUUID()]);
}

export async function loadProductionJob(db: DatabaseClient, id: string): Promise<ProductionJob | null> {
  const { rows } = await db.query<{ body: unknown }>("SELECT body FROM campaign_production_jobs WHERE id=$1 AND organization_id=$2", [id, AURENDOR_ORGANIZATION_ID]);
...
export async function listProductionJobs(db: DatabaseClient): Promise<ProductionJob[]> {
  const { rows } = await db.query<{ body: unknown }>("SELECT body FROM campaign_production_jobs WHERE organization_id=$1 ORDER BY month,id", [AURENDOR_ORGANIZATION_ID]);
...
export async function saveProductionJob(db: DatabaseClient, before: ProductionJob, after: ProductionJob, actor = "codex-production"): Promise<void> {
  ...
  await db.transaction(async tx => {
    await tx.query("SELECT id FROM campaign_production_jobs WHERE id=$1 AND organization_id=$2 FOR UPDATE", [before.id, AURENDOR_ORGANIZATION_ID]);
    ...
    const result = await tx.query(`UPDATE campaign_production_jobs SET body=$1::jsonb, revision=$2,stage=$3,updated_at=now()
      WHERE id=$4 AND organization_id=$5 AND revision=$6`, [JSON.stringify(after), after.revision, after.stage, after.id, AURENDOR_ORGANIZATION_ID, before.revision]);
    if (result.rowCount !== 1) throw new Error("Production changed concurrently. Reload before retrying.");
    await tx.query("UPDATE content_items SET status=$1,scheduled_at=$4,updated_at=now() WHERE id=$2 AND organization_id=$3", [after.stage === "APPROVED" ? "APPROVED" : after.stage === "OWNER_REVIEW" ? "NEEDS_REVIEW" : "DRAFT", after.id, AURENDOR_ORGANIZATION_ID, after.slot.scheduledAt]);
...
export async function importCampaignPlan(db: DatabaseClient, raw: unknown): Promise<{ created: boolean; count: number }> {
  ...
  return db.transaction(async tx => {
    const inserted = await tx.query(`INSERT INTO campaign_production_plans (organization_id,month,plan_hash,body)
      VALUES ($1,$2,$3,$4::jsonb) ON CONFLICT DO NOTHING RETURNING month`, [AURENDOR_ORGANIZATION_ID, plan.month, planHash, JSON.stringify(plan)]);
    if (!inserted.rowCount) {
      const existing = await tx.query<{ plan_hash: string }>("SELECT plan_hash FROM campaign_production_plans WHERE organization_id=$1 AND month=$2", [AURENDOR_ORGANIZATION_ID, plan.month]);
    ...
    [`campaign-${plan.month}`, AURENDOR_ORGANIZATION_ID, plan.month, plan.rationale, JSON.stringify({ source: "campaign-production-v1", planHash })]);
    const strategy = await tx.query<{ id: string }>("SELECT id FROM monthly_strategies WHERE organization_id=$1 AND month=$2", [AURENDOR_ORGANIZATION_ID, plan.month]);
    for (const slot of plan.slots) {
      const prior = await tx.query<{ month: string; scheduled_at: Date | string }>("SELECT month,scheduled_at FROM content_items WHERE id=$1 AND organization_id=$2", [slot.id, AURENDOR_ORGANIZATION_ID]);
