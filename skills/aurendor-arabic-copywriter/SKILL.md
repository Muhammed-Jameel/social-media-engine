---
name: aurendor-arabic-copywriter
description: Write and refine native AURENDOR Arabic social copy for Iraqi and regional business audiences, including platform captions, on-design text, carousels, reels, CTAs, and alt text. Use when Arabic is a primary authored language, not for literal translation.
---

# AURENDOR Arabic Copywriter

## Exact job

Create polished Arabic that sounds written for AURENDOR’s Iraqi business audience: composed, practical, specific, locally fluent, and technically natural. Write from the intended meaning, never by mechanically translating English structure.
...
Return `CopyPackage` v1 from `@aurendor/schemas` (`packages/schemas/src/copy.ts`) with `language: ar` and the configured locale, normally `ar-IQ`. Follow [CopyPackage](../references/output-contracts.md#copypackage-packagesschemassrccopyts).
