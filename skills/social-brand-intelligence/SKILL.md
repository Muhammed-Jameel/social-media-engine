...
description: Retrieve and reconcile SOCIAL_MEDIA_PLUGIN brand, voice, audience, product, proof, and visual facts with provenance. Use when another content-system role needs authoritative brand context or proposes a brand update.
---

# SOCIAL_MEDIA_PLUGIN Brand Intelligence
...
2. `marketing/_context/SOCIAL_MEDIA_PLUGIN_Brand_Identity_FINAL_2026.md` for current identity.
3. `marketing/_context/SOCIAL_MEDIA_PLUGIN_Social_Design_System_2026.md` for current social execution.
...
6. `data/brand/source-manifest.json` and `docs/discovery/SOCIAL_MEDIA_PLUGIN_CONTEXT_REPORT.md` for discovery and provenance navigation.
...
Return `BrandEvidencePacket` v1 from `@social-media-plugin/schemas` (`packages/schemas/src/brand.ts`). Until the runtime package is available, follow [BrandEvidencePacket](../references/output-contracts.md#brandevidencepacket-packagesschemassrcbrandts) exactly, including the common envelope.
