---
name: social-brand-compliance
description: Gate SOCIAL_MEDIA_PLUGIN copy, claims, design, assets, and approval class against effective brand truth and publishing policy. Use for a structured pass/revise/reject/escalate decision, not for owner approval or content generation.
---

# SOCIAL_MEDIA_PLUGIN Brand Compliance

## Exact job

Evaluate a content candidate against the effective SOCIAL_MEDIA_PLUGIN brand version, claims policy, asset/licensing rules, accessibility requirements, and configured approval policy. Identify exact violations and required actions without silently rewriting the candidate or granting authorization.
...
Return `ComplianceDecision` v1 from `@social-media-plugin/schemas` (`packages/schemas/src/compliance.ts`). Follow [ComplianceDecision](../references/output-contracts.md#compliancedecision-packagesschemassrccompliancets).
