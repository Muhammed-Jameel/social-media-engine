...
No third-party skill or script executes merely because it exists. Adoption requires a named purpose, source/license review, permission review, injection/exfiltration check, pinned version where possible, and a measurable quality or reliability benefit on an SOCIAL_MEDIA_PLUGIN fixture.

## Skills used during implementation

| Skill | Source | Decision | Reason and boundary |
|---|---|---|---|
| SOCIAL_MEDIA_PLUGIN Marketing Team Lead | Local SOCIAL_MEDIA_PLUGIN skill | Adopted for build coordination | Enforces brand/evidence/execution quality and specialist routing; no external scripts |
...
The engine vendors small SOCIAL_MEDIA_PLUGIN-specific skills under `skills/social-*`: brand intelligence, content strategy, Arabic copy, English copy, art direction, social design, design critique, brand compliance, social publishing, social analytics, experimentation, monthly retrospective, and next-month planning.
...
- Third-party Arabic copy/design prompts were not installed because the authoritative SOCIAL_MEDIA_PLUGIN brand and current social design system already provide stronger task-specific evidence, and no candidate has yet demonstrated benchmark lift.
