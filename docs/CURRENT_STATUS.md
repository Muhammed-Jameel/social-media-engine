...
**Maturity:** `OFFLINE` moving to supervised Postiz onboarding / creative benchmarking

**External publishing:** Disabled; no verified successful publication

**Creative production:** Paused by a persisted `BENCHMARKING` release gate
...
- Self-hosted Postiz deployment, typed Public API client, server-only API-key boundary, verified-asset upload cache, duplicate-safe publication batches, provider-specific package adaptation, and a supervised **Publishing** dashboard for the five requested networks.
...
- The owner has not yet created the Postiz administrator/API key or completed Meta, LinkedIn, TikTok, and X provider-application/OAuth account binding and canary verification.
- Production asset hosting, notifications, managed PostgreSQL backup/restore, and live monitoring are not verified.
- The Postiz adapter is implemented, but live dispatch remains fail-closed until account capabilities, production authentication, durable gates, and a supervised canary are verified.
- The complete representative creative suite has not passed every current-hash professional, originality, feed, technical, rights, and owner gate. Production remains durably paused.
...
3. Repair candidate `03`, break the `07`/`12` feed repetition, revise the other below-bar candidates, and restart every applicable exact-hash critic/originality/feed review.
4. Create and lock the local Postiz administrator, configure provider applications, connect each owned account, and add the Postiz Public API key to Content OS.
